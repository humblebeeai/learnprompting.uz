---
sidebar_position: 10
title: "🔴 LLMlarni kalibrlash"
---


# LLMlarni kalibrlash

Ba’zi bir tarafkashliklarni LLMlar namoyon qiladigan holatlarni **natija** ni kalibrlash orqali bartaraf etish mumkin
taqsimotlar**(@zhao2021calibrate).

**Chiqarish taqsimotini kalibrlash aynan nimani anglatadi?**

Keling, tezkor misolni ko‘rib chiqamiz: Aytaylik, bizda ikki mumkin bo‘lgan yorliq, `Ijobiy` va `Salbiy` bo‘lgan  sentiment analysis  vazifasi bor.
 LLM  ga `Kirish: hech narsa Sentiment: ` bilan prompt berilganda nima sodir bo‘lishini ko‘rib chiqing.
...
Chunki `hechnarsa` na ijobiy, na salbiy tushuncha hisoblanadi, biz LLM dan `Ijobiy` va `Salbiy` uchun taxminan 0.5 ehtimollikni kutamiz. Biroq, ko‘pincha (va ushbu misolda ham) bu holat kuzatilmaydi.

```
p("Ijobiy" | "Kirish: hech narsa Sentiment:") = 0.9

p("Salbiy" | "Kirish: hech narsa Sentiment:") = 0.1
```

Ushbu kontekstdan mustaqil kirish uchun ushbu yorliq ehtimollari berilgan bo‘lsa, biz bilamizki, LLM ning
**Natijalar taqsimoti** ehtimol tarafkash bo‘ladi
`Ijobiy` yorlig‘iga tomon. Bu LLMning `Ijobiy` ni afzal ko‘rishiga olib kelishi mumkin
...
kiritmalar uchun ham `Ijobiy`, ham `Salbiy` uchun 0.5 ehtimollik belgilanadi,
shunda biz ko‘pincha `Ijobiy` ga bo‘lgan og‘ishlarni olib tashlashimiz mumkin va LLM yanada ishonchli bo‘ladi
...
namuna ham `Ijobiy`, ham `Salbiy` sifatida tasniflanmoqda:

```
Kirish: Men bu filmni yomon ko‘raman. — Sentiment: Salbiy
Kirish: Men bu filmni yaxshi ko‘raman. — Sentiment: Ijobiy
Kirish: M/A — Sentiment: Ijobiy
Kirish: M/A — Sentiment: Salbiy
Kirish: hech narsa — Sentiment: Ijobiy
Kirish: hech narsa — Sentiment: Salbiy
Kirish: Men tuxumni yoqtiraman. — Sentiment:
```
...
`Kirish: hech narsa Sentiment: ` har ikkala yorliq uchun ham taxminan 0.5 ehtimollik belgilanadi.
...
bu misol GPT-3 bilan takrorlab bo‘lmaydi, chunki uni faqat `Ijobiy` va `Salbiy` yorliqlari bilan cheklab bo‘lmaydi.
...
```
q("Ijobiy" | "Kirish: hech narsa Sentiment::") = 0.5

q("Salbiy" | "Kirish: hech narsa Sentiment::") = 0.5
```
